...
def restart_server():
    ...
    def delayed_restart():
        ...
        if sys.platform == "win32":
        ...
        elif os.path.exists("/.dockerenv") or os.environ.get("PYGUARDDNS_DOCKER") == "1":
            os._exit(0)
        else:
            ...
            os.execl("/bin/sh", "/bin/sh", restart_script)
...
.sm-modal{background:#0d1424;border:1px solid rgba(55,75,100,.45);border-radius:.85rem;width:min(92vw,940px);height:85vh;display:flex;flex-direction:column;box-shadow:0 12px 48px rgba(0,0,0,.5),0 0 0 1px rgba(100,130,180,.06);animation:smSlideUp .2s ease}
...
@media(max-width:980px){.sm-grid{grid-template-columns:1fr}.sm-modal{width:96vw;height:90vh}}
</style>
    ...
    sections = [
        ("Status & Dashboard", [
            ...
            ("GET", "/api/stats/cache", "Cache statistics.",
             None,
             '{\n  "size": 1234, "hits": 5600, "misses": 1200,\n  "hit_rate": 82.4, "negative_entries": 45\n}'),
            ("GET", "/api/explain?domain=...&amp;client=...", "Decision explanation for a domain with step-by-step path.",
             None,
             '{\n  "domain": "doubleclick.net", "result": "block",\n  "reason": "block rule", "matched_rule": "bs::doubleclick.net",\n  "matched_list": "HaGeZi", "allow_rule_won": false,\n  "steps": [{"step": "profile_allow", ...}, ...],\n  "client_ip": "192.168.0.80", "profile_name": "Default"\n}'),
        ]),
        ("Query Log", [
            ...
            ("POST", "/api/querylog/rule-action", "Create a rule from a query log entry (one-click allow/block).",
             ...
             '{"ok": true, "scope": "global", "action": "block", "pattern": "ads.example.com", "rule": "bd::ads.example.com"}'),
        ]),
        ("Filtering", [
        ...
        ("Blocklists", [
            ...
            ("GET", "/api/blocklists/stats", "Blocklist statistics.",
             None,
             '{"total_lists": 3, "total_rules": 250000, "enabled_lists": 3}'),
            ...
            ("GET", "/api/blocklists/{id}/report", "Cache info and import report for a blocklist.",
             None,
             '{\n  "cache_file": "...", "rule_count": 85000,\n  "import_report": {"valid": 85000, "duplicate": 1200, "regex": 45}\n}'),
        ...
        ("Cosmetic Lists", [
            ("GET", "/api/cosmeticlists", "All blocklists that contain cosmetic rules.",
             None,
             '[{"id": 1, "name": "EasyList", "cosmetic_count": 320}]'),
            ...
            ("GET", "/api/cosmeticlists/{id}", "Cosmetic rules for a specific blocklist.",
             None,
             '["example.com##.ad-banner", "##.tracking-pixel"]'),
            ...
            ("POST", "/api/cosmeticlists/log", "Log cosmetic rule application from the browser userscript.",
             '{"domain": "example.com", "rule": "##.ad-banner"}',
             '{"ok": true}'),
        ]),
        ("Clients", [
            ...
            ("GET", "/api/clients/{id}", "Single client details.",
             None,
             '{"id":1,"name":"PC-1","ip":"192.168.0.10","profile_id":1,"profile_name":"Default","filtering_enabled":1}'),
        ...
        ("Profiles", [
            ...
            ("GET", "/api/profiles/{id}/services", "Blocked services for a profile.",
             None,
             '[{"service": "youtube"}, {"service": "tiktok"}]'),
            ...
            ("GET", "/api/services", "List all available service names for service blocking.",
             None,
             '["youtube", "tiktok", "instagram", "facebook", "discord", "twitch", "netflix", "spotify", "steam", "roblox", ...]'),
        ]),
        ("Upstreams", [
            ...
            ("GET", "/api/upstreams/health", "Health status for all upstreams.",
             None,
             '[{"id":1,"name":"Cloudflare","healthy":true,"latency_ms":12.4,"consecutive_failures":0,"paused":false}]'),
        ...
        ("Backup &amp; Restore", [
            ...
            ("POST", "/api/restore", "Restore from a backup file.",
             ...
             '{"ok": true, "counts": {"settings": 12, "rules": 45, "blocklists": 3}}'),
            ("POST", "/api/restore-preview", "Preview what a backup restore would change.",
             'multipart/form-data with "file" field',
             '{\n  "ok": true,\n  "settings": 12, "rules": 45, "blocklists": 3,\n  "upstreams": 2, "profiles": 3, "clients": 5\n}'),
        ]),
        ("Updates", [
        ...
        ("Metrics", [
            ...
            ("GET", "/api/runtime_metrics", "DNS hot-path counters as JSON (cache hits/misses, filter decisions, upstream errors, query-log drops, queue sizes).",
             None,
             '{\n  "cache_hit_total": 5600, "cache_miss_total": 1200,\n  "filter_allow_total": 8000, "filter_block_total": 3400,\n  "upstream_error_total": 12, "qlog_drop_total": 0,\n  "unknown_client_dropped_total": 0\n}'),
