...
def test_handle_dns_request_logs_connection_type(monkeypatch):
    settings = {"disable_ipv6": "0", "query_log_enabled": "1", "lan_only": "0", "filtering_enabled": "0", "cache_enabled": "0", "upstream_mode": "sequential"}
    seen = {}
    monkeypatch.setattr(app, "get_setting", lambda key, default="": settings.get(key, default))
    monkeypatch.setattr(app, "ensure_client", lambda client_ip: None)
    ...
    monkeypatch.setattr(app, "forward_query", lambda req, timeout_override=None: (req, "test"))

    app.handle_dns_request(_query("example.com", app.QTYPE_CODE["A"]), "127.0.0.1", "HTTPS")
